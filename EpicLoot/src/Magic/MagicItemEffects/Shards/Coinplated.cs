...
{
    // Golden (Fortune) chest shard effect: armour yourself in coin. Each incoming hit commits up to
    // PoolPercent of the purse; the rolled shard value is the efficiency (0.5..0.9) at which those coins
    // convert into absorbed damage, so the pool blocks (purse * PoolPercent% * efficiency) damage and the
    // player is charged only for what was actually absorbed:
    //
    ...
    // damage, Mythic ~1.11). Mirrors EitrShield -- scales the pre-armour hit on the local player, and the
