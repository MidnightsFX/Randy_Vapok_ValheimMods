...
All data is sourced from [`config/shardstones.json`](config/shardstones.json), keyed by shard color â†’ slot â†’ effect.
Standard shards (Core/Dark/Light) define one effect per broad slot; Boss shards use a single **uniform** effect that
applies to any socket. Effect names are the internal `EffectType` ids, and each scales by rarity (Magic â†’ Rare â†’ Epic â†’
Legendary â†’ Mythic).
...
|---|---|---|---|---|---|---|---|---|---|
| **Red** (Vitality) â˜‘ï¸ | LifeGainOnHit | LifeGainOnHit | HealthOnEitrUse | ModifyHealthRegen | IncreaseHealth | PercentHealth | BulkUp | DamageTakenGivesAdrenaline | AddHealthRegen |
| **Yellow** (Stamina) â˜‘ï¸ | ModifyAttackStaminaUse | ModifyDrawStaminaUse | EnergeticEitr | PercentStamina | ModifyStaminaRegen | IncreaseStamina | StaminaOnKill | UseAdrenalineAsStamina | ModifySprintStaminaUse |
...
## Boss shards (uniform â€” one effect on any slot)

| Shard | Rarity | Effect (all slots) |
|---|---|---|
| **Eikthyr** | Rare | ShockingCharge â˜‘ï¸ |
| **Elder** | Rare | ForestsAid â˜‘ï¸ |
| **Bonemass** | Epic | CorpseRot â˜‘ï¸ |
| **Moder** | Epic | IcyRetribution â˜‘ï¸|
| **Yagluth** | Legendary | MeteorSummoner â˜‘ï¸ |
| **Queen** | Legendary | Everflow â˜‘ï¸ |
| **Fader** | Mythic | NecroticFire â˜‘ï¸|
...
- `ResolveCategory` first maps a host item to a *fine* type (Swords, Bows, Bucklers, etc.), then falls back to its
  group â€” so, e.g., a sword and a club both pick up the `MeleeWeapon` effect since no fine-type effects are defined.
...
- The fine type itself comes from [`ItemTypeClassifier`](src/GatedItemType/ItemTypeClassifier.cs), the mod-wide
  answer to "which `iteminfo.json` type is this item?" â€” the item's configured entry when it has one, else a
